"use client";

import Image from "next/image";
import { X } from "lucide-react";

import IconButton from "@/components/ui/IconButton";

import { Data } from "@/types";
import Currency from "@/components/ui/Currency";
import useCart from "@/hooks/useCart";

interface CartItemProps {
    data: Data
}

const CartItem: React.FC<CartItemProps> = ({ data }) => {
    const cart = useCart();

    const onRemove = () => {
        cart.removeItem(data.id)
    }

    return (
        <li className="flex py-6 border-b">
            <div className="relative h-24 w-24 rounded-md overflow-hidden sm:h-48 sm:w-48 bg-gray-100">
                <Image
                    fill
                    src={`${process.env.PUBLIC_URL}${data?.attributes?.images?.data[0]?.attributes.url}`}
                    alt="Image"
                    className="aspect-square object-contain rounded-md mix-blend-darken" />
            </div>
            <div className="relative ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                <div className="absolute z-10 right-0 top-0">
                    <IconButton onClick={onRemove} icon={<X size={16} />} />
                </div>
                <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                    <div className="flex justify-between">
                        <p className="text-lg font-semibold text-black">
                            {data.attributes.title}
                        </p>
                    </div>
                    <div className="mt-1 flex text-sm h-fit">
                        <p className="text-gray-500">{data.attributes.colors.data[0].attributes.title}</p>
                        <p className="text-gray-500 ml-4 border-l border-gray-200 pl-4">{data.attributes.sizes.data[0].attributes.title}</p>
                    </div>
                    <Currency value={data.attributes.price} />
                </div>
            </div>
        </li>
    )
}

export default CartItem